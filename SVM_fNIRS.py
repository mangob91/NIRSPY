...
NIRSPY = NIRSPY_basic(dataPath, sampling_rate)
...
#%%
labels = HbO[:,HbO.shape[1]-1]
features = HbO[:, :HbO.shape[1] -1]
tscv = NIRSPY_analysis(6)
...
drift_removed = np_pre.baseline_drift(X_train[5], 4)
extracted_feature = np_pre.feature_extraction(drift_removed)
label_final = np_pre.feature_extraction_label(Y_train[5])
features_final = np_pre.feature_selection(extracted_feature, label_final, 12)
#%%
...
temp = na.bootStrapping(temp_features, [0.5,0.5],200,200)
# boot strapping based on algorithm given from professor Xiaofu He
print(sum(temp[:,12] == 1)/len(temp))
#%%
